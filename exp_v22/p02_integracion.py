"""exp_v22: integración = pipeline v11 + features de OCURRENCIA (hurdle).
...
exp_v22/p01. Optuna 30 + fijo. 2 submits. Reporta gain de occ_*.
"""
...
focc = pd.read_parquet(RUTA_EXP / "features_ocurrencia_v22.parquet")
...
def armar(cut: int, con_clase: bool) -> pd.DataFrame:
    ...
    d = d.join(focc_c[cut], how="left")          # ocurrencia (v12)
    return d
...
print(f"backtest multi-corte fijo: {evaluar(PARAMS_FIJOS):.4f} (v16 clasificador 0.2363)")

# pata del ensamble = params FIJOS (config del ganador v16)
...
print(f"gain occ_* (regresor de ocurrencia): {occ_gain:.1f}%")
...
print(f"corr(LGBM v22, OLS): {np.corrcoef(pred_lgbm[comun], ols[comun])[0,1]:.4f} (v16 clasif: 0.9931)")
...
def submit(nombre, factor):
    ...
    f = RUTA_EXP / f"exp_v22_{nombre}.csv"; out.write_csv(f)
    subprocess.run([str(RUTA_PROYECTO / ".venv/bin/kaggle"), "competitions", "submit",
                    "-c", COMPETENCIA, "-f", str(f), "-m", f"exp_v22 regresor-ocurrencia {nombre}"], check=True)
    print(f"submit OK -> {f.name}")


submit("ens_sin_calib", 1.00)   # efecto del cambio clasif->regresor, limpio
submit("ens_con_calib", 1.03)   # comparable al ganador 0.226
print("\nlisto")
