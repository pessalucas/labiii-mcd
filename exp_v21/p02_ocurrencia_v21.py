...
w_par_rq = panel(sellin.group_by("customer_id","product_id","periodo").agg(pl.col("cust_request_qty").sum().alias("tn")), ["customer_id","product_id"])
w_par_rt = panel(sellin.group_by("customer_id","product_id","periodo").agg(pl.col("cust_request_tn").sum().alias("tn")), ["customer_id","product_id"])
...
def _pend(M):
    ...
    sl = (y * t).sum(axis=1) / (t ** 2).sum()
    sl[np.isnan(M).any(axis=1)] = np.nan
    return sl
...
def armar_corte(cut, con_target):
    ...
    # --- REQUEST a nivel par (v21): intención de compra ---
    RQ = w_par_rq.reindex(idx)[cols12].values
    RT = w_par_rt.reindex(idx)[cols12].values
    pidio = RQ > 0
    d["pair_req_1m"] = np.nan_to_num(pidio[:, 0]).astype(int)
    d["pair_req_2m"] = np.nan_to_num(pidio[:, :2]).any(1).astype(int)
    d["pair_req_3m"] = np.nan_to_num(pidio[:, :3]).any(1).astype(int)
    prom_rq = np.nanmean(RQ[:, :12], axis=1)
    d["pair_req_qty_0"] = RQ[:, 0] / (prom_rq + EPS)
    RQn = RQ / (prom_rq[:, None] + EPS)
    d["pair_req_pend3"] = _pend(RQn[:, :3][:, ::-1])
    # fill-rate: ¿pide más de lo que recibe? (demanda insatisfecha del par, 3m)
    d["pair_req_fillrate"] = np.nansum(RT[:, :3], 1) / (np.nansum(A[:, :3], 1) + EPS)
    if con_target:
        obj = mes_menos(cut, -2)
        d["y"] = (w_par.reindex(idx)[obj].fillna(0).values > 0).astype(int) if obj in w_par.columns else np.nan
        d = d.dropna(subset=["y"])
    return d
...
def objetivo(trial):
    params = dict(objective="binary", random_state=SEMILLA, verbosity=-1, subsample_freq=1,
    ...
    m = lgb.LGBMClassifier(**params); m.fit(d_tune[FEATURES], d_tune["y"])
    return roc_auc_score(d_val["y"], m.predict_proba(d_val[FEATURES])[:, 1])


study = optuna.create_study(direction="maximize", sampler=optuna.samplers.TPESampler(seed=SEMILLA))
study.optimize(objetivo, n_trials=40, show_progress_bar=False)
BEST = dict(objective="binary", random_state=SEMILLA, verbosity=-1, subsample_freq=1, **study.best_params)
print(f"optuna best AUC (50% val, n=40): {study.best_value:.4f} | {study.best_params}")


def fit(d): m = lgb.LGBMClassifier(**BEST); m.fit(d[FEATURES], d["y"]); return m


m_early, m_late = fit(d_early), fit(d_late)
m_full = fit(pd.concat([d_early, d_late]))
auc = roc_auc_score(d_late["y"], m_early.predict_proba(d_late[FEATURES])[:, 1])
print(f"AUC (train early->eval late, 100%): {auc:.4f} (v16 sin request fue 0.793)")
...
def feat_producto(cut, modelo):
    ...
    tmp = pd.DataFrame({"product_id": d.index.get_level_values(1), "p": p,
                        "dem": p * d["monto_mean"].values})
    agg = tmp.groupby("product_id").agg(occ_n_esperados=("p", "sum"), occ_demanda_esp=("dem", "sum"))
...
feat.to_parquet(RUTA_EXP / "features_ocurrencia_v21.parquet")

m_full.booster_.save_model(str(RUTA_EXP / "modelo_ocurrencia_v21.txt"))
joblib.dump(m_full, RUTA_EXP / "modelo_ocurrencia_v21.pkl")
json.dump(study.best_params, open(RUTA_EXP / "hiperparametros_clasificador.json", "w"), indent=2)
