"""exp_v12: integración = pipeline v11 + features de OCURRENCIA (hurdle).
...
exp_v12/p01. Optuna 30 + fijo. 2 submits. Reporta gain de occ_*.
"""
...
focc = pd.read_parquet(RUTA_EXP / "features_ocurrencia.parquet")
...
def submit(nombre: str, params: dict) -> None:
    ...
    f = RUTA_EXP / f"exp_v12_{nombre}.csv"
    out.write_csv(f)
    subprocess.run([str(RUTA_PROYECTO / ".venv/bin/kaggle"), "competitions", "submit",
                    "-c", COMPETENCIA, "-f", str(f), "-m", f"exp_v12 v11+ocurrencia {nombre}"],
                   check=True)
...
(RUTA_EXP / "feature_importance_integracion.txt").write_text(
    "exp_v12 - FI del modelo de producto v11 + ocurrencia\n" + "\n".join(fi_txt))
print("\nFI guardado")
